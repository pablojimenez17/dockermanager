...
const OrganizationDashboard = () => {const { t } = useTranslation();
  ...
  if (!activeOrg) {
    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center h-[60vh]">
                ...
                <h2 className="text-2xl font-bold mb-3 text-slate-900 dark:text-white">{t("auto.no_active_organization")}</h2>
        ...
        <div className="flex items-center gap-4">
            ...
            className="bg-brand-600 hover:bg-brand-700 text-white px-6 py-3 rounded-sm flex items-center gap-2 font-bold shadow-sm active:scale-95">
                            ...
                            <Plus size={20} /> {t("auto.create_organization")}
                        </button>
                    </div>
        }
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-sm w-full max-w-md overflow-hidden shadow-md animate-fade-in">
                            <div className="p-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50">
                                <h3 className="font-bold flex items-center gap-2 text-lg text-slate-800 dark:text-slate-200">
                                    <Building2 size={20} className="text-brand-500" /> {t("auto.new_organization")}
                                </h3>
                            ...
                            <form onSubmit={handleCreateOrg} className="p-6 space-y-4">
                                <div>
                                    <label className="block text-sm font-bold mb-1.5 text-slate-700 dark:text-slate-300">{t("auto.organization_name")}</label>
                                    <input
                  required
                  type="text"
                  value={orgData.name}
                  ...
                  placeholder={t("auto.e_g_acme_corp")} />
  ...
  if (loading) return <div className="p-8">{t("auto.loading_organization_data_")}</div>;
    ...
    <div className="space-y-8 pb-12">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-700/50 pb-6">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">
                        {activeOrg.name} {t("auto.organization")}
                    </h1>
        ...
        <div className="flex gap-3">
            ...
            className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 px-4 py-2 rounded-sm flex items-center gap-2 font-medium transition-colors">
                            ...
                            <Shield size={18} /> {t("auto.new_role")}
                        </button>
                        <button
            ...
            className="bg-brand-600 hover:bg-brand-700 text-white px-4 py-2 rounded-sm flex items-center gap-2 font-medium transition-colors shadow-sm shadow-brand-500/20">
                            ...
                            <Mail size={18} /> {t("auto.invite_member")}
                        </button>
                    </div>
        }
            </div>
        ...
        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-sm p-6 shadow-sm">
                        <h3 className="text-lg font-bold flex items-center gap-2 mb-6 text-slate-800 dark:text-slate-200 border-b border-slate-100 dark:border-slate-700 pb-3">
                            <Users className="text-brand-500" /> {t("auto.team_members_")}{members.length})
                        </h3>
            ...
            <div key={r._id} className="border border-slate-200 dark:border-slate-700 rounded-sm p-4">
                                <div className="flex items-center justify-between mb-3">
                                    <h4 className="font-semibold text-slate-900 dark:text-white flex items-center gap-2">
                                        {r.name}
                                        {r.name === 'Owner' && <span className="bg-amber-100 text-amber-600 text-[10px] px-2 py-0.5 rounded-full uppercase tracking-wider font-bold">{t("auto.system")}</span>}
                                    </h4>
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-sm p-6 shadow-sm">
                        <h3 className="text-lg font-bold flex items-center gap-2 mb-6 text-slate-800 dark:text-slate-200 border-b border-slate-100 dark:border-slate-700 pb-3">
                            <Mail className="text-amber-500" /> {t("auto.pending_invitations")}
                        </h3>
                        <div className="overflow-x-auto">
                            <table className="w-full text-left text-sm whitespace-nowrap">
                                <thead>
                                    <tr className="text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-700">
                                        <th className="pb-3 px-2">{t("auto.email")}</th>
                                        <th className="pb-3 px-2">{t("auto.role")}</th>
                                        <th className="pb-3 px-2">{t("auto.expires")}</th>
                                        <th className="pb-3 px-2">{t("auto.action")}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {invites.map((inv) =>
      ...
      <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-sm w-full max-w-md overflow-hidden">
                        <div className="p-4 border-b border-slate-200 dark:border-slate-700 flex justify-between">
                            <h3 className="font-bold text-lg">{t("auto.invite_team_member")}</h3>
                            <button onClick={() => setIsInviteModalOpen(false)}>✕</button>
                        </div>
                        <form onSubmit={handleInviteUser} className="p-6 space-y-4">
                            <div>
                                <label className="block text-sm font-medium mb-1">{t("auto.email_address")}</label>
                                <input required type="email" value={inviteData.email} onChange={(e) => setInviteData({ ...inviteData, email: e.target.value })} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-sm p-2.5 outline-none focus:ring-2 ring-brand-500" placeholder={t("auto.colleague_company_com")} />
                            </div>
                            <div>
                                <label className="block text-sm font-medium mb-1">{t("auto.assign_role")}</label>
                                <select required value={inviteData.roleId} onChange={(e) => setInviteData({ ...inviteData, roleId: e.target.value })} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-sm p-2.5 outline-none focus:ring-2 ring-brand-500">
                                    <option value="" disabled>{t("auto.select_a_role")}</option>
      ...
      <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-sm w-full max-w-lg overflow-hidden max-h-[90vh] flex flex-col">
                        ...
                        <form onSubmit={handleCreateRole} className="p-6 space-y-6 overflow-y-auto custom-scrollbar">
                            <div>
                                <label className="block text-sm font-bold mb-1">{t("auto.role_name")}</label>
                                <input required type="text" value={roleData.name} onChange={(e) => setRoleData({ ...roleData, name: e.target.value })} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-sm p-2.5 outline-none focus:ring-2 ring-brand-500 font-medium" placeholder={t("auto.e_g_devops_engineer")} />
                            </div>

                            <div>
                                <label className="block text-sm font-bold mb-3 border-b border-slate-100 dark:border-slate-700 pb-2">{t("auto.permissions_overview")}</label>
                                <div className="space-y-3">
