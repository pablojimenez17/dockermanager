...
const Secrets = () => {const { t } = useTranslation();
  ...
  if (loading) return <div className="p-8 text-center animate-pulse text-slate-500">{t("auto.decrypting_vault_")}</div>;
    ...
    <div className="p-4 md:p-8 pb-20 max-w-7xl mx-auto space-y-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 dark:text-white flex items-center space-x-3">
                        ...
                        <span>{t("auto.secret_manager")}</span>
                    </h1>
                    <p className="text-slate-600 dark:text-slate-400 mt-2 max-w-2xl leading-relaxed">
                        {t("auto.store_sensitive_data_like_api_keys_and_d")}
                    </p>
                </div>
                <button
      ...
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-sm p-6 shadow-sm animate-in fade-in slide-in-from-top-4">
                    <div className="flex items-center space-x-3 text-amber-600 dark:text-amber-500 mb-6 bg-amber-50 dark:bg-amber-500/10 p-4 rounded-sm border border-amber-200 dark:border-amber-500/20">
                        ...
                        <span className="text-sm font-medium">{t("auto.warning_for_your_security_the_secret_val")}</span>
                    </div>

                    <form onSubmit={handleCreateSecret} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{t("auto.secret_name_uppercase_recommended_")}</label>
                  ...
                  placeholder={t("auto.e_g_mysql_root_password")}
                  className="pl-10 w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500 dark:text-white transition-all font-mono" />
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{t("auto.secret_value")}</label>
                                <div className="relative">
                                    <input
                  ...
                  placeholder={t("auto.paste_your_sensitive_token_here")}
                  ...
                  className="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-slate-600 dark:hover:text-slate-300">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{t("auto.description_optional_")}</label>
                            <input
              type="text"
              value={newSecret.description}
              onChange={(e) => setNewSecret({ ...newSecret, description: e.target.value })}
              placeholder={t("auto.what_is_this_used_for_")}
            ...
            <div className="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-sm overflow-hidden shadow-sm">
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wider">
                                <th className="p-5 font-bold">{t("auto.secret_name")}</th>
                                <th className="p-5 font-bold">{t("auto.description")}</th>
                                <th className="p-5 font-bold">{t("auto.created_at")}</th>
              ...
              <tr>
                                    <td colSpan="4" className="p-8 text-center text-slate-500 dark:text-slate-400">
                                        ...
                                        <p className="text-lg font-medium">{t("auto.vault_is_empty")}</p>
                                        <p className="text-sm">{t("auto.store_your_first_encrypted_token_to_use_")}</p>
                                    </td>
                                </tr> :
              ...
              <tr key={secret._id} className="hover:bg-slate-50 dark:hover:bg-slate-700/20 transition-colors group">
                                        ...
                                        <td className="p-5 line-clamp-1">{secret.description || <span className="text-slate-400 italic">{t("auto.no_description")}</span>}</td>
