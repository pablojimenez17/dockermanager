...
import {
  ...
  Code, Server, Database, Mail, CheckCircle2, Send, Star, Quote } from
'lucide-react';
import { useTheme } from '../components/ThemeContext';

/* ─── Optimized Background ───────────────────────────── */
const AnimatedBackground = () => {const { t } = useTranslation();
    ...
    <motion.div
      ...
      className="relative lg:col-span-3 rounded-sm overflow-hidden border border-slate-200/50 dark:border-slate-700/50 shadow-sm shadow-brand-500/5 dark:shadow-black/20 bg-white/50 dark:bg-slate-900/50 backdrop-blur-md group">
            ...
            <div className="relative flex items-center gap-2 px-4 py-3 bg-slate-100/80 dark:bg-slate-800/80 border-b border-slate-200/50 dark:border-slate-700/50">
                ...
                <span className="ml-2 text-xs text-slate-500 dark:text-slate-400 font-mono flex-1 text-center mr-8">{t("auto.orbit_cli")}</span>
            </div>
        ...
        className="fixed top-0 left-0 right-0 z-50 border-b border-slate-200/50 dark:border-slate-800/50 bg-white/70 dark:bg-slate-950/70 backdrop-blur-xl shadow-sm">
                ...
                <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
                    <Link to="/" className="flex items-center gap-3 group">
                        ...
                        <span className="text-lg font-black tracking-wide text-slate-900 dark:text-white">{t("auto.orbit")}</span>
                    </Link>
            ...
            className="max-w-4xl mx-auto">
                        ...
                        <motion.h1 variants={fadeInUp} className="text-5xl md:text-7xl lg:text-8xl font-black tracking-tight mb-8 leading-[1.1]">
                            {t("auto.manage_infrastructure")} <br className="hidden md:block" />
                            <span className="text-brand-500">
                                {t("auto.without_friction_")}
                            </span>
                        </motion.h1>
                        ...
                        <motion.p variants={fadeInUp} className="text-lg md:text-xl text-slate-600 dark:text-slate-400 mb-10 max-w-2xl mx-auto leading-relaxed">
                            {t("auto.a_powerful_intuitive_orchestration_platf")}
            ...
            className="grid grid-cols-1 lg:grid-cols-5 gap-12 items-center">
                        ...
                        <motion.div variants={fadeInUp} className="lg:col-span-2">
                            <h2 className="text-3xl md:text-5xl font-black mb-6 text-slate-900 dark:text-white">
                                {t("auto.built_for")} <span className="text-brand-500">{t("auto.developers")}</span>{t("auto._designed_for")} <span className="text-indigo-500">{t("auto.humans")}</span>.
                            </h2>
                            <p className="text-lg text-slate-600 dark:text-slate-400 mb-6 leading-relaxed">
                                {t("auto.orbit_removes_the_complexities_of_docker")}
            ...
            className="text-center mb-16">
                        ...
                        <span className="text-brand-500 font-bold uppercase tracking-widest text-sm mb-4 block">{t("auto.platform_capabilities")}</span>
                        <h2 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white max-w-2xl mx-auto">
                            {t("auto.everything_you_need_to_ship_production_r")}
                        </h2>
                    </motion.div>
            ...
            className="mb-16">
                        ...
                        <motion.div variants={fadeInUp} className="max-w-2xl">
                            <span className="text-indigo-500 font-bold uppercase tracking-widest text-sm mb-4 block">{t("auto.built_for_scale")}</span>
                            <h2 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white mb-4">
                                {t("auto.one_platform_infinite_possibilities_")}
                            </h2>
                            <p className="text-lg text-slate-600 dark:text-slate-400">
                                {t("auto.whether_you_re_running_a_personal_blog_o")}
                            </p>
                        </motion.div>
            ...
            className="text-center mb-16">
                        ...
                        <span className="text-emerald-500 font-bold uppercase tracking-widest text-sm mb-4 block">{t("auto.loved_by_engineering_teams")}</span>
                        <h2 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white max-w-3xl mx-auto">
                            {t("auto.don_t_just_take_our_word_for_it_")}
                        </h2>
                    </motion.div>
            ...
            <section className="relative max-w-7xl mx-auto px-6 py-20 border-t border-slate-200/50 dark:border-slate-800/50">
                <div className="text-center mb-16">
                    <span className="text-brand-500 font-bold uppercase tracking-widest text-sm mb-4 block">{t("auto.pricing")}</span>
                    <h2 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white mb-4">{t("auto.simple_transparent_pricing")}</h2>
                    <p className="text-slate-500 dark:text-slate-400 max-w-lg mx-auto">{t("auto.start_for_free_scale_when_you_re_ready_")}</p>
                </div>
                ...
                'bg-slate-50 dark:bg-slate-800/80'}`
                  ...
                  <div className={`inline-flex text-[10px] font-bold uppercase tracking-wide rounded-full px-2 py-0.5 mb-2 ${p.highlighted ? 'bg-white/20 text-white' : 'bg-brand-100 dark:bg-brand-500/15 text-brand-600 dark:text-brand-400'}`}>
                                        ...
                                        <div className={`text-2xl font-black mt-1 ${p.highlighted ? 'text-white' : 'text-slate-900 dark:text-white'}`}>
                                            {p.price}<span className={`text-sm font-normal ${p.highlighted ? 'text-brand-200' : 'text-slate-400 dark:text-slate-500'}`}>{t("auto._mo")}</span>
                                        </div>
                                    </th>
                )}
                            </tr>
          ...
          className="max-w-5xl mx-auto rounded-sm bg-slate-900 border border-slate-800 p-10 md:p-20 text-center relative shadow-md">
                    ...
                    <div className="relative z-10">
                        <h2 className="text-4xl md:text-6xl font-black text-white mb-6">
                            {t("auto.ready_to_modernize_your_infrastructure_")}
                        </h2>
                        <p className="text-xl text-slate-400 mb-10 max-w-2xl mx-auto">
                            {t("auto.join_thousands_of_developers_using_orbit")}
                        </p>
                ...
                className="w-full sm:w-auto px-8 py-4 bg-slate-800 hover:bg-slate-700 text-white font-bold rounded-sm transition-all border border-slate-700 flex items-center justify-center gap-2">
                                ...
                                <Mail size={18} /> {t("auto.contact_sales")}
                            </a>
                        </div>
                    </div>
                </motion.div>
            ...
            <footer className="border-t border-slate-200/50 dark:border-slate-800/50 py-10 px-6">
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="font-black text-lg text-slate-900 dark:text-white">{t("auto.orbit")}</span>
                    </div>
                    <div className="text-slate-500 dark:text-slate-400 text-sm font-medium text-center md:text-left">
                        © {new Date().getFullYear()} {t("auto.orbit_pablo_jim_nez_prieto_")}
                    </div>
                    <div className="flex items-center gap-4">
