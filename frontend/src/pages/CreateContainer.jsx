...
const CreateContainer = () => {const { t } = useTranslation();
                        ...
                        <option key={net.Id} value={net.Name}>{net.Name}</option>
                                                    ...
                                                    {c.networkMode !== 'none' && (
                                                      <div className="mt-3">
                                                        ...
                                                        {/* Chips of added extra networks */}
                                                        <div className="flex flex-wrap gap-1.5 mb-2">
                                                          {(c.extraNetworks || []).map((extraNet) => (
                                                            <span key={extraNet} className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-brand-100 dark:bg-brand-900/40 text-brand-700 dark:text-brand-300 text-xs font-mono border border-brand-200 dark:border-brand-700">
                                                              {extraNet}
                                                              <button
                                                                type="button"
                                                                onClick={() => updateContainer(c.id, 'extraNetworks', (c.extraNetworks || []).filter(n => n !== extraNet))}
                                                                className="text-brand-500 hover:text-red-500 transition-colors ml-0.5"
                                                              >✕</button>
                                                            </span>
                                                          ))}
                                                        </div>

                                                        {/* Selector to add a new extra network */}
                                                        <select
                                                          value=""
                                                          onChange={(e) => {
                                                            if (!e.target.value) return;
                                                            const already = (c.extraNetworks || []).includes(e.target.value);
                                                            if (!already && e.target.value !== c.networkMode) {
                                                              updateContainer(c.id, 'extraNetworks', [...(c.extraNetworks || []), e.target.value]);
                                                            }
                                                          }}
                                                          className="w-full px-3 py-1.5 border border-dashed border-gray-300 dark:border-slate-600 rounded bg-white dark:bg-slate-900 text-sm text-gray-500"
                                                        >
                                                          <option value="">+ Add another network...</option>
                                                          <option value="bridge">Bridge (Default VPC)</option>
                                                          {availableNetworks
                                                            .filter(n => !['host', 'none'].includes(n.Name) && n.Name !== c.networkMode && !(c.extraNetworks || []).includes(n.Name))
                                                            .map(net => <option key={net.Id} value={net.Name}>{net.Name}</option>)
                                                          }
                                                        </select>
                                                      </div>
                                                    )}
