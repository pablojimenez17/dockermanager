...
const ViewContainers = () => {const { t } = useTranslation();
  ...
  const handleAction = async (id, action) => {
    try {
      ...
      if (action === 'delete') addToast('Success', 'Container deleted', 'success');
  ...
  return (
    <div className="text-gray-900 dark:text-slate-100">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 space-y-4 sm:space-y-0">
                <div>
                    <h1 className="text-2xl font-bold mb-1">{t("auto.instances")}</h1>
                    <p className="text-sm text-gray-500 dark:text-slate-400">{t("auto.view_and_manage_your_container_fleet_")}</p>
                </div>
                <div className="flex w-full sm:w-auto space-x-3">
                    <div className="relative">
                        ...
                        <input
              type="text"
              placeholder={t("auto.search_instances_")}
              value={searchQuery}
      ...
      <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 text-sm p-4 rounded mb-6 flex items-center">
                ...
                <table className="table-industrial w-full">
                    <thead>
                        <tr>
                            ...
                            <th>{t("auto.image")}</th>
                            <th>{t("auto.ports")}</th>
                            <th className="text-right">{t("auto.actions")}</th>
                        </tr>
                    </thead>
                    <tbody>
                        {loading ?
            <tr>
                                <td colSpan="6" className="text-center py-10">
                                    ...
                                    <span className="text-sm text-gray-500">{t("auto.loading_instances_")}</span>
                                </td>
                            </tr> :
            filteredContainers.length === 0 ?
            <tr>
                                <td colSpan="6" className="text-center py-10">
                                    <span className="text-sm text-gray-500">{t("auto.no_instances_found_")}</span>
                                </td>
                            </tr> :
                  ...
                  '-'}
                    ...
                    <>
                                                        <div className="relative group/tooltip">
                                                            ...
                                                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover/tooltip:block bg-slate-800 text-white text-[10px] px-2 py-1 rounded whitespace-nowrap z-[100] font-medium shadow-sm">{t("auto.stop_instance")}</div>
                                                        </div>
                                                        <div className="relative group/tooltip">
                                                            ...
                                                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover/tooltip:block bg-slate-800 text-white text-[10px] px-2 py-1 rounded whitespace-nowrap z-[100] font-medium shadow-sm">{t("auto.redeploy")}</div>
                                                        </div>
                                                        <div className="relative group/tooltip">
                                                            ...
                                                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover/tooltip:block bg-slate-800 text-white text-[10px] px-2 py-1 rounded whitespace-nowrap z-[100] font-medium shadow-sm">{t("auto.open_terminal")}</div>
                                                        </div>
                                                    </> :
                    ...
                    <div className="relative group/tooltip">
                                                        ...
                                                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover/tooltip:block bg-slate-800 text-white text-[10px] px-2 py-1 rounded whitespace-nowrap z-[100] font-medium shadow-sm">{t("auto.start_instance")}</div>
                                                    </div>
                    }
                                                ...
                                                <div className="relative group/tooltip">
                                                    ...
                                                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover/tooltip:block bg-slate-800 text-white text-[10px] px-2 py-1 rounded whitespace-nowrap z-[100] font-medium shadow-sm">{t("auto.view_logs")}</div>
                                                </div>
                                                    ...
                                                    <div className="origin-top-right absolute right-0 mt-1 w-48 rounded shadow-sm bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 ring-1 ring-black ring-opacity-5 invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all z-[100]">
                                                        <div className="py-1">
                                                            ...
                                                            <button onClick={() => {if (window.confirm(`Delete ${container.name}?`)) handleAction(container._id, 'delete');}} className="flex w-full items-center px-4 py-2 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 border-t border-gray-100 dark:border-slate-700">
                                                                <Trash2 size={14} className="mr-2" /> {t("auto.delete")}
                                                            </button>
                                                        </div>
                                                    </div>
                                                </div>
              ...
              <tr className="bg-gray-50/50 dark:bg-slate-800/30">
                                            ...
                                            <td colSpan="5" className="px-4 py-4">
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 text-sm">
                                                    <div>
                                                        <h4 className="font-semibold text-gray-900 dark:text-white mb-2 flex items-center"><Network size={14} className="mr-2 text-gray-500" /> {t("auto.network")}</h4>
                                                        <div className="space-y-1 text-gray-600 dark:text-slate-400">
                                                            <div className="flex justify-between"><span className="text-gray-500">{t("auto.internal_ip")}</span> <span className="font-mono">{containerStats[container._id]?.ipv4Address || container.ipv4Address || '-'}</span></div>
                                                            <div className="flex justify-between"><span className="text-gray-500">{t("auto.mode")}</span> <span className="font-mono">{containerStats[container._id]?.networkMode || container.networkMode || 'bridge'}</span></div>
                                                        </div>
                                                    </div>
                                                    <div>
                                                        <h4 className="font-semibold text-gray-900 dark:text-white mb-2 flex items-center"><Activity size={14} className="mr-2 text-gray-500" /> {t("auto.telemetry")}</h4>
                                                        {container.state === 'running' ?
                      containerStats[container._id] ?
                      <div className="space-y-2">
                                                                    <div>
                                                                        <div className="flex justify-between text-xs mb-1"><span className="text-gray-500">{t("auto.cpu_usage")}</span><span className="font-mono">{containerStats[container._id].cpuPercent}%</span></div>
                                                                    ...
                                                                    <div>
                                                                        <div className="flex justify-between text-xs mb-1"><span className="text-gray-500">{t("auto.memory")}</span><span className="font-mono">{(containerStats[container._id].memUsage / 1024 / 1024).toFixed(1)} {t("auto.mb")}</span></div>
                      ...
                      <div className="text-xs text-gray-500 flex items-center"><RefreshCw className="animate-spin mr-1" size={12} /> {t("auto.loading_stats_")}</div> :
                      <div className="text-xs text-gray-500">{t("auto.offline")}</div>}
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
      ...
      <div className="fixed inset-0 bg-gray-900/50 dark:bg-slate-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-slate-800 w-full max-w-md rounded shadow-sm overflow-hidden">
                        <div className="px-6 py-4 border-b border-gray-200 dark:border-slate-700 flex items-center">
                            ...
                            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{t("auto.redeploy_instance")}</h3>
                        </div>
                        <div className="p-6 text-sm text-gray-600 dark:text-slate-300 space-y-4">
                            <p>{t("auto.this_will_pull_the_latest")} <strong>{redeployConfirm.image}</strong> {t("auto.image_and_restart")} <strong>{redeployConfirm.name}</strong>.</p>
                            <p className="text-amber-600 dark:text-amber-500">{t("auto.warning_ephemeral_data_inside_the_contai")}</p>
                        </div>
                        <div className="px-6 py-4 bg-gray-50 dark:bg-slate-800/50 flex justify-end space-x-3">
                            <button onClick={() => setRedeployConfirm(null)} className="btn-secondary">{t("auto.cancel")}</button>
                            <button onClick={() => {handleRedeploy(redeployConfirm.id, redeployConfirm.name, redeployConfirm.image);setRedeployConfirm(null);}} className="btn-primary">{t("auto.confirm_redeploy")}</button>
                        </div>
                    </div>
                </div>
      }
      ...
      <div className="fixed inset-0 bg-gray-900/50 dark:bg-slate-900/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-slate-800 w-full max-w-md rounded shadow-sm overflow-hidden">
                        <div className="px-6 py-4 border-b border-gray-200 dark:border-slate-700 flex items-center">
                            ...
                            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{t("auto.create_snapshot")}</h3>
                        </div>
                        <form onSubmit={submitSnapshot} className="p-6">
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t("auto.snapshot_tag")}</label>
                            ...
                            <div className="mt-6 flex justify-end space-x-3">
                                <button type="button" onClick={() => setSnapshotModalOpen(false)} className="btn-secondary">{t("auto.cancel")}</button>
                                <button type="submit" disabled={!snapshotName} className="btn-primary">{t("auto.create_snapshot")}</button>
                            </div>
                        </form>
                    </div>
                </div>
